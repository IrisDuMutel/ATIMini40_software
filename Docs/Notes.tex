...
In this works, the barometer bmp280 has been used. It can widely measure pressure ranging from 300 mbar to 1100 hPa, with an accuracy of $\pm$1 hPa \cite{bmp280}. The temperature accuracy is $\pm$1.0\textdegree C.

In \autoref{fig:ardu_bmp280}, we can see the connection necessary to obtain the data from this sensor using the I2C wiring:

\begin{figure}[h!]
	\centering
	\includegraphics[width=0.8\textwidth]{Images/bmp280.png}
	\caption{I2C connection between arduino UNO and bmp280 sensor}
	\label{fig:ardu_bmp280}
\end{figure}

 In the case of the arduino Mega, the SCK pin from the sensor must be conncted to pin SCL (pin 21) and the SDI pin from the sensor to the SDA pin from the arduino (pin 20). The SPI interface is also available for this sensor.
...
The quadcopter used in the ground/ceiling/wall effect tests is depicted in figure \autoref{fig:quad} and is constituted by a metallic frame that enables different distancing between the motors. The ESC are the ones discussed in \autoref{sec:ESC}, which can be powered by a 2s LiPo battery (7.4 V) up to an 8s (33.6 V). These controllers draw a maximum of 50 A.

\begin{figure}[h!]
	\centering
	\includegraphics[width=0.8\textwidth]{Images/quad.jpg}
	\caption{Quadcopter fram and motors with 10"x4.5" propellers}
	\label{fig:quad}
\end{figure}

More than one motor type is used since different propeller sizes are tested trying to stay close to a ratio of 2 between the diameter and the pitch. The proposed propellers are the following:
...
\subsection{Castle Phoenix Edge 50A ESC}
\label{sec:ESC}
...
The rest of the cables are connected to a potentiometer that acts as radio controller. The \textbf{calibration} is supposed to be automatic as stated by the manufacturer in the \hyperref{https://www.castlecreations.com/en/phoenix-edge-50-esc-010-0102-00}{category}{name}{product's site}.

\subsection{Motors}

The three motors chosen for testing are the following:

\begin{itemize}
	\item \hyperref{https://www.altitudehobbies.com/products/leopard-2830-12t-980kv-brushless-airplane-motor}{category}{name}{Leopard 2830-12T 980Kv} Brushless Airplane Motor. Used for the 10"x4.5" propellers (see \autoref{fig:table_980kv}).
	\item \hyperref{https://www.altitudehobbies.com/collections/leopard-airplane-motors/products/leopard-2835-12t-690kv-brushless-airplane-motor}{category}{name}{Leopard 2835-12T 690kv} Brushless Airplane Motor. For the intermediate sized propellers  (see \autoref{fig:table_690kv}).
	\item \hyperref{https://store.tmotor.com/goods-334-MN3110.html}{category}{name}{T-motor MN3110 470Kv} brushless motors. For the bigger propellers (see \autoref{fig:table_470kv}).
\end{itemize}


\begin{figure}[h!]
	\centering
	\includegraphics[width=0.9\textwidth]{Images/chart980Kv_all.png}
	\caption{Leopard 2830-12T 980Kv Brushless Airplane Motor specifications}
		\label{fig:table_980kv}
	\end{figure}


\begin{figure}[h!]
	\centering
	\includegraphics[width=0.9\textwidth]{Images/chart690Kv_all.png}
	\caption{Leopard 2835-12T 690kv Brushless Airplane Motor specifications}
	\label{fig:table_690kv}
\end{figure}

\begin{figure}[h!]
	\centering
	\includegraphics[width=0.9\textwidth]{Images/chart470Kv_all.png}
	\caption{T-motor MN3110 470Kv specifications}
	\label{fig:table_470kv}
	\end{figure}

Given the 12V and 90A limitation of the power supply, we are going to stay in a safe temperature and current range when it comes to the bigger motors (470Kv) which would require a higher voltage that would then draw lower values of current.

\section{Power supply}

The system needs more than one source of power to feed all boards, sensors and the motors. 
...
The tachometers are powered by a Keysight triple output DC power supply (see \autoref{fig:tach_power}). A constant voltage of 10V is supplied to the sensors leaving the current bounded. 

\begin{figure}[h!]
	\centering
	\includegraphics[width=0.5\textwidth]{Images/tach_power.jpeg}
	\caption{Keysight Triple Output DC Power Supply}
	\label{fig:tach_power}
\end{figure}
...
Notice that raw data looks like in \autoref{fig:unfiltered_noprops}:
...
This is why filters are also a scope of this study\footnote{For a MATLAB example on filters described in this section, look at \hyperref{https://github.com/IrisDuMutel/ATIMini40_software/blob/master/MATLAB/Scripts/Examples/example_FFTandFilters.m}{category}{name}{the example script}}. The F/T sensor currently in use provides unfiltered analog data that is sampled through a DAQ, and therefore transformed into digital information. However, this digital data stream may contain noise from vibration, EM interferences... In \cite{Conyers2019}, an averaging filter is implemented. The tests were all performed using a sampling rate of 1 kHz, and an averaging size of 250 samples. This resulted in an effective sampling rate of 40 Hz.

In the current work, a different approach is used. Data is sampled at 150 Hz from all 6 channels. The data is directly logged into a text file that is later or processed on MATLAB.

Using MATLAB functions such as \textit{lowpass}, \textit{highpass}, \textit{bandpass} and others, one can obtain first approaches to a good filtered solution. 
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=0.7\textwidth]{Images/butter_filters_2.eps}
	\caption{Butterworth filters of orders 1 to 6}
	\label{fig:butter}
\end{figure}

A cutoff frequency of 1 Hz was imposed and the sample frequency is supposed to be 200 Hz. 
...
When applied to a force vector coming from the F/T sensor, we can see the comparison between a generic low pass filter (cutoff 0.5Hz) from MATLAB and a Butterworth 5$_{th}$ order filter in \autoref{fig:butter_vs_lowpass}:

\begin{figure}[h!]
	\centering
	\includegraphics[width=0.7\textwidth]{Images/butter5_vs_lowpass.eps}
	\caption{Butterworth filter compared to lowpass filter in MATLAB}
	\label{fig:butter_vs_lowpass}
\end{figure}

As noticed in the picture, noise is removed successfully while a little delay is added.


\subsubsection{Chebyshev filters (type I)}

Using Chebyshev filters 

\begin{figure}[h!]
	\centering
	\includegraphics[width=0.7\textwidth]{Images/cheby1.eps}
	\caption{Chebyshev filters of orders 2 to 6}
	\label{fig:cheby1}
\end{figure}

\begin{figure}[h!]
	\centering
	\includegraphics[width=0.7\textwidth]{Images/cheby5_vs_lowpass.eps}
	\caption{Chebyshev (type I) filter compared to lowpass filter in MATLAB}
	\label{fig:cheby_vs_lowpass}
\end{figure}


\subsection{Automatic detection of steps}


\subsection{Offset removal}

This part of the process is in charge of finding offsets in all datafiles and removing it. 

In every test step where Fz is supposed to be 0, an offset must be removed so that the following steps are as accurate as possible. 
